...
import { apiPostCreateComment } from "@/api/postNewsFeed/postCreateComment";
import { apiPostEditComment } from "@/api/postNewsFeed/postEditComment";
import { apiPostDeleteComment } from "@/api/postNewsFeed/postDeleteComment";
...
export default function CommentScreen() {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      const res = await apiGetDetailComment(postId);
  ...
  const postComment = async (content: string) => {
    ...
    try {
      const token = await AsyncStorage.getItem("access_token");
      if (!token) throw new Error("Chưa đăng nhập");
      const playload : CreateCommentPayload = {
        post_id: postId,
        content,
        user_tags: [],
        medias: []
      ...
      const res = await apiPostCreateComment(playload);
  ...
  const updateComment = async (commentId: number | string, content: string) => {
    ...
    try {
      ...
      const res = await apiPostEditComment(payload)
  ...
  const removeComment = async (commentId: string) => {
    try {
      ...
      const res = await apiPostDeleteComment(commentId)
