...
export default function PostMenu({ postId, onPostDeleted }: PostMenuProps) {
  ...
  const fetchPostById = async (id: number) => {
    try {
      ...
      const res = await axios.get(
        `https://beta.api.gateway.overate-vntech.com/api/v1/kaizen/${id}/detail-post`,
        {
          headers: {
  ...
  const handleRemove = async () => {
    try {
      ...
      const res = await axios.get(
        `https://beta.api.gateway.overate-vntech.com/api/v1/kaizen/${postId}/detail-post`,
        {
          headers:
          {
            Authorization: `Bearer ${token}`,
      ...
      if (currentUserId !== post.user_created.id) {
        Toast.show({
          type: "error",
          text1: "Thông báo",
          text2: "Bạn chỉ có thể xóa bài viết của chính mình",
        });
        setMenuVisible(false);
        return;
      }
            ...
            onPress: async () => {
              try {
                const deleteRes = await axios.post(
                  `https://beta.api.gateway.overate-vntech.com/api/v1/kaizen/${postId}/remove-post`,
                  {},
