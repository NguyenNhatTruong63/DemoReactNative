...
import { apiGetAttendanceDetail1 } from "@/api/attendance/getAttendanceDetail1";
...
export default function AttendanceDetailScreen() {
  ...
  const checkCheckoutStatus = async () => {
    const lastDateStr = await AsyncStorage.getItem("last_checkout_date");
  ...
  const fetchAttendanceList = async (dateObj?: Date) => {
    ...
    try {
      ...
      const res = await apiGetAttendanceDetail1(dateToSend, userId)
      ...
      <View style={styles.fabContainer}>
        <View style={styles.fabItem}>
          ...
          <TouchableOpacity
            style={[
              styles.fabButton,
              { backgroundColor: checkedOutToday ? "#ccc" : "#007bff" },
            ]}
            onPress={() => {
              if (!checkedOutToday) router.push("/timekeeping/cameraScreen");
            }}
        ...
        <View style={styles.fabItem}>
          <TouchableOpacity
            style={[styles.fabButton, { backgroundColor: "#007bff" }]}
            onPress={() => router.push("/timekeeping/attendanceHistory")}
          >
